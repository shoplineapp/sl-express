...
class App {
  ...
  loadFramework() {
    ...
    this.context.Redis = Redis

    Logger.sharedLogger.init(this.config.logger)
    this.context.Logger = Logger

    this.context.MessageQueue = MessageQueue
  ...
  loadConfig() {
    ...
    let filePath = path.resolve(process.cwd(), 'config')

    let configFile = this.importDir(filePath);

    this.config = deepExtend({}, configFile);

  }

  loadServices() {

    let filePath = path.resolve(process.cwd(), 'api/services')

    this.services = this.importDir(filePath);
  ...
  loadMiddlewares() {
    ...
    let filePath = path.resolve(process.cwd(), 'api/middlewares')
    this.middlewares = this.importDir(filePath)
