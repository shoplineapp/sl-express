...
const assignRecursiveMappings = function(requiredMappings) {
  let mappings = {}
  const _assign = (klass, key, prefix="") => {
    ...
    if (typeof klass === 'object') {
      ...
      Object.keys(klass).forEach((key) => {
        ...
        if (typeof klass[key] === 'object') { prefix = `${prefix}${inflection.capitalize(key)}${this.config.app.moduleDelimiter}` }
      ...
      mappings[`${prefix}${key}`] = new klass(this)
...
class App {
  ...
  loadControllers(app) {
    ...
    let controllers = this.importDir(filePath, { recursive: true });

    this.controllers = assignRecursiveMappings.call(this, controllers)
