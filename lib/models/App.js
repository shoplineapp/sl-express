...
class AppHelper {
  ...
  loadRecursively(filePath, handler = function(klass, key) {}) {

    let klasses = this.importDir(filePath, { recursive: true })

    let delimiter = this.app.config.app.moduleDelimiter
  ...
  importDir(dirname, options={}) {
    ...
    try { result = require('require-all')(Object.assign(options, { dirname })) }
    catch(e) { }
...
class App {
  ...
  loadModels() {
    ...
    let result = helper.loadRecursively(filePath, function(klass, key) {
  ...
  loadControllers(app) {
    ...
    let result = helper.loadRecursively(filePath, function(klass) {
  ...
  loadHelpers() {
    ...
    this.helpers = deepExtend({},
      helper.importDir(path.resolve(__dirname, '../helpers')),
      helper.importDir(path.resolve(process.cwd(), this.config.app.directory, 'helpers'))
    )
