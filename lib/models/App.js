...
class App {
  ...
  /*
   * @description A getter of the mongoose library.
   *
   * @method mongoose
   *
  ...
  get redis() {
    //assuming this will only be got after prepare phase
    if (!this._redis) {
  ...
  get messageQueue() {
    if (!this._messageQueue) {
      this._messageQueue = new MessageQueue(this.config.messageQueue)
    }
  ...
  loadFramework() {
    ...
    this.models = AppHelper.importDir(
      path.resolve(__dirname, '../models'),
      { required: false }
    )

    this.services = AppHelper.importDir(
      path.resolve(__dirname, '../services'),
      { required: false }
    )

    this.logger = new this.services.Logger(this.config.logger)
  ...
  loadPlugins() {
    ...
    const plugins = AppHelper.importPlugins(
      dirPath,
      { required: false, only: this.config.app.plugins }
    )

    const existingKeys = Object.keys(plugins)
    const remainingKeys = this.config.app.plugins.filter( (key) => {
      return !existingKeys.includes(key)
    })

    this.plugins = AppHelper.importPlugins(
      path.resolve(__dirname, '../plugins'),
      { required: false, only: remainingKeys, }
    )
  ...
  async connectMessageQueue() {
    ...
    try {
      ...
      await this.messageQueue.init(this.config.messageQueue).connect()
  ...
  async connectRedis() {
    ...
    try { await this.redis.connect() } catch(e) { throw e }
  ...
  async asyncExecutePluginPhase(phase) {
    return Promise.each( this.config.app.plugins, (key) => {
      const plugin = this.plugins[key]

      if (typeof plugin[phase] === 'function') {
        return plugin[phase](this)
      }
  ...
  prepare() {
    ...
    this.executePluginPhase('didLoadFramework')
    ...
    this.executePluginPhase('prepare')
  ...
  async connectDependencies() {
    await Promise.each( this.config.app.plugins, (plugin) => {
      return this.plugins[plugin].connectDependencies(this)
    })
  ...
  async disconnectDependencies() {
    const inversedPlugin = this.config.app.plugins.slice().reverse()

    await Promise.each(inversedPlugin, (plugin) => {
      return this.plugins[plugin].disconnectDependencies(this)
    })
  ...
  async start() {
    ...
    await this.asyncExecutePluginPhase('willStartService')

    await this.startService()

    await this.asyncExecutePluginPhase('didStartService')
