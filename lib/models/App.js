...
class App {
  ...
  // TODO: refactor executePluginPhase and asyncExecutePluginPhase
  executePluginPhase(phase, reversed = false) {
    const pluginConfig = this.config.app.plugins
    const plugins = reversed ? pluginConfig.slice().reverse() : pluginConfig
    ...
    plugins.forEach( (key) => {
      ...
      try {
        if (!plugin) {
          const err = new Error('plugin not exist')
  ...
  async asyncExecutePluginPhase(phase, reversed = false) {
    const pluginConfig = this.config.app.plugins
    const plugins = reversed ? pluginConfig.slice().reverse() : pluginConfig
  ...
  async connectDependencies() {
    await this.asyncExecutePluginPhase('connectDependencies')
  ...
  async disconnectDependencies() {
    await this.asyncExecutePluginPhase('disconnectDependencies', true)
