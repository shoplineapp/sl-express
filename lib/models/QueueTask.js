 ...
 *
 */
let taskTypeEnum = Object.keys(require('../../config/handlerRoutes.js'))
...
class QueueTask {
  ...
  static async queue(taskObj, priority = 'default') {
    ...
    let queue = mq.queue.default
    if (priority == 'high')  { queue = mq.queue.highPriority }

    await mq.queueMessage(queue, new Buffer(JSON.stringify({ queueTaskId: task.id })))
