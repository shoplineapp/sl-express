...
class GRpcServer {
  constructor(ip, port, extras) {
    const {
      packageName,
      autoloadPaths,
      setupHealthCheck = false,
      setupReflection = false,
    } = extras;
    this.ip = ip;
    this.port = port;
    this.services = {};
    ...
    this.setupReflection = setupReflection;
    ...
    if (setupReflection && !grpcReflectionPkg) {
      const { ReflectionService } = require("@grpc/reflection");
      grpcReflectionPkg = { ReflectionService };
    }
  }
  ...
  configureReflection() {
    if (!this.setupReflection) return;
    if (!grpcReflectionPkg || !this.server) return;

    const definition = Definition.getDefinitions();
    const reflectionService = new grpcReflectionPkg.ReflectionService(definition);
