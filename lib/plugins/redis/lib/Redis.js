...
class Redis {
  ...
  init(config) {
      ...
      return function (...args) {
        return new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            reject(new Error(`Redis operation "${originalMethod.name}" timed out`));
          }, config.timeoutMs);
    ...
    Promise.promisifyAll(this.redis.RedisClient.prototype, { promisifier: customPromisifier });
    Promise.promisifyAll(this.redis.Multi.prototype, { promisifier: customPromisifier });
